<?php 
	...
	if (isset($_REQUEST["Type"])){
		...
		if ($_REQUEST["Type"]== "Add"){
			...
			$sql = "INSERT Into tbl_sinhvien (MaSV,MaLop,HoTen,NgaySinh,GioiTinh,QueQuan,Email) VALUES ('".$masv."','".$malop."','".$hoten."', 
					 	 '".$ngaysinh."','".$gioitinh."',
					 	'".$quequan."','".$email."')";
		...
		if ($_REQUEST["Type"]== "Update"){
			...
			$sql = "UPDATE tbl_sinhvien SET HoTen='".$hoten."',NgaySinh='".$ngaysinh."', GioiTinh='".$gioitinh."', MaLop='".$malop."',QueQuan='".$quequan."', Email='".$email."' 
					 WHERE MaSV='".$masv."'";
	...
	if (isset($_REQUEST["MaSV"])){
		$masv = $_REQUEST["MaSV"];
		$sql = "DELETE FROM tbl_sinhvien WHERE MaSV='wqeqw'";
		$result = $db->query($sql);
		if ($result && $db->affected_rows > 0){
			echo "OK";
		}else{
	...
	$sql = "SELECT COUNT(*) FROM tbl_sinhvien WHERE MaLop='".$maLop."'";
	$result = $db->query($sql);
	$total_row = 0;
	if ($result){
	...
	$sql = "SELECT * FROM tbl_sinhvien WHERE MaLop='".$maLop."' LIMIT 0, $last";
	$result = $db->query($sql);
	...
	<table class="ds">
			...
			<tbody>
				...
				while ( $row = $result->fetch_array () ) {
					...
					echo "<button name='btnXoaSV' class='btnXoaSV' value='" . $row ["MaSV"] . "' ><span class='ui-icon ui-icon-trash'  ></span> </button>";
					echo "</td>";
					echo "</tr>";
				}
				$result->free ();
